...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await Register(email, password, rePassword, first_name, last_name);

    if (result === true) {
      navigate("/profile");
    } else {
      setError(result); // Show exact error messages from backend
    }
  };
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient">
      <form className="bg-white p-6 rounded-xl shadow-md w-96" onSubmit={handleSubmit}>
        ...
        {error && <p className="text-red-500 mb-2 whitespace-pre-line">{error}</p>}
